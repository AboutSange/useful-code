...
def python_copy(copy_source, copy_target, mode='copy', ignore_errors=True, retry_count=1, log_obj=None):
    """
    ...
    log_print('[{0}]{1} --> {2}'.format(mode, copy_source, copy_target), log_obj=log_obj)
    ...
    while count <= retry_count:
        try:
            ...
            is_error = True
            log_print('[{0} error]{1}'.format(mode, e), log_obj=log_obj)

        count += 1

    if (not ignore_errors) and is_error:
        exit_with_log('python_copy error exit', log_obj=log_obj)
